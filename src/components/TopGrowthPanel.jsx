...
import { computeGrowthByProduk } from '../utils/aggregate.js';

export default function TopGrowthPanel({ posisiRowsFiltered }) {
  const [mode, setMode] = useState('mtm');
  const growth = useMemo(() => computeGrowthByProduk(posisiRowsFiltered, mode), [posisiRowsFiltered, mode]);

  return (
    <Panel
      title="TOP &amp; BOTTOM GROWTH PRODUK OSL"
      right={
